...
import (
	...
	"zeroflowui" // Импорт вашей библиотеки
)
...
func (b UINotificationButton) DispatchTouch(pipe *zeroflowui.SystemPipelineDecorator, timeline *zeroflowui.UIEventFlow, tx, ty byte, signal *zeroflowui.TextSignal) {
	if tx >= b.XMin && tx <= b.XMax && ty >= b.YMin && ty <= b.YMax {
		// Изменяем Payload атомарного контейнера сигналов библиотеки
		signal.Payload = "OK"
	...
	app.Main(func(a app.App) {
		for e := range a.Events() {
			...
			if _, ok := e.(paint.Event); ok {
				...
				// ------------------------------------------------------------
				// ПОДКЛЮЧЕНИЕ ГОТОВЫХ МЕТОДОВ ИЗ КАТАЛОГА СТРИМА (stream.go)
				// ------------------------------------------------------------
				...
				// Шаг 1: Читаем первый символ ленивого стрима логов
				charStr, nextStream, isEnd = charStream()
				if !isEnd && charStr != "" {
					// Извлекаем byte из 1-символьной строки без аллокаций памяти
					atlasChain.RenderGlyph(glCtx, charStr[0], startX, startY, textScale)

					// Шаг 2: Читаем второй символ из остатка потока
					charStr, nextStream, isEnd = nextStream()
					if !isEnd && charStr != "" {
						atlasChain.RenderGlyph(glCtx, charStr[0], startX+20, startY, textScale)
					}
				}
				// ------------------------------------------------------------
